...
obligations are proved from the shipped source and the remaining
\allocOpen{} are named.  Along the way the kernel's unary numerals, which
...
check, \texttt{tid + 1} rewritten so it cannot overflow --- and checked
...
is proved by that paper's own recipe against the same generated model.  Of
...
The open ones are \allocOpenAllAdds{} \texttt{u64} additions.  Each fits
whenever the surrounding code admits it --- \texttt{used + n <= sizes[heap]}
was tested, and a size is a \texttt{u64} --- but saying so is arithmetic
about the maximum, which splitting and computing does not do.  A test pins
the open set, so a new open obligation fails it and so does closing one
quietly.  The model test for the Python allocator notes that its model
answers 0 past the end of an array where the compiled C reads out of
bounds; the port has no such read, and the obligations are the proof of
that.
...
\item \textbf{Not yet arithmetic.}  An obligation that holds because
  \texttt{a < 1000} and \texttt{b < 1000} give \texttt{a + b < 2000} is
  out of reach of split-and-compute.  The lemmas an interval tactic would
  build on --- commutativity, two-sided monotonicity of $+$ and $\times$,
  subtract-then-add cancellation --- are not in the prelude yet.  With
  literals in the kernel, that tactic can now compare against the real
  \texttt{u32::MAX}, and it is the next step.
...
\item \textbf{Known gaps, asserted.}  \texttt{class\_covers} in
  \texttt{regs.rs} (\classCoversProved) needs reasoning about the range a
  guard establishes; a record contract through a field update is not yet
  proved.  Each has a test asserting it is still open, so closing it cannot
  happen quietly, and each is checked at runtime meanwhile.
\end{itemize}
