...
confirmed and the source now avoids.  The rest of LeanOS --- the region
list, the loader's header checks, threads and the loader --- is ported too,
and the prover now goes through loops by their own \texttt{\#[invariant]}:
\portProved{} of \portObligations{} obligations across \portFunctions{}
functions and \portLoops{} loops, every port answering as its Python does.
The two theorems LeanOS is founded on --- a region list the checker accepts
has no two overlapping regions, and an address belongs to at most one
owner --- are proved about the region list as lifted from Rust, stated term
for term as they were about the hand-typed model.
Every one of the \rustCertificates{} Rust theorems
...
\section{Loops, and the rest of LeanOS}\label{sec:loops}

Nearly every function LeanOS still had in Python is a loop over the region
list, and the automation of Section~\ref{sec:bounds} did not go through a
loop at all: \texttt{xs[i]} behind \texttt{i < xs.len()} was open when the
comparison was the loop's condition.  \texttt{read\_procedure} already
stated a loop's four obligations --- the invariant holds on entry, a guarded
pass keeps it, the variant comes down, and the loop finishes --- and the
hand-written models of the LeanOS papers discharged them one shape at a
time.  \texttt{by\_loop} discharges them for any shape the facts reach:
each by \texttt{by\_bounds}, after \texttt{by\_state} splits the loop's
state tuple into named fields with one \texttt{Prod.ind} per \texttt{mk}.
The invariant at exit and the failed loop condition then become two facts,
and the postcondition is proved with them in hand.

The invariant used is the one written, with one addition the prover makes
for the proof and not the theorem.  An early \texttt{return} is lowered as
state --- \texttt{\_returned}, \texttt{\_return\_value} --- and the lowered
loop goes on iterating with the returned value frozen, so what the loop
keeps need only hold while nothing has returned: the written invariant
\texttt{X} is used as \texttt{\_returned or X}, conjoined with
\texttt{not \_returned or post(\_return\_value)} for a contract and
\texttt{\_returned or \_ok} for a panic obligation.  The theorem's statement
never mentions an invariant; the kernel checks all four obligations for the
one used.  An invariant too weak to carry the goal is not strengthened
beyond that, and the goal stays open --- a count of matches is bounded by
the length only if the invariant says the count is bounded by \texttt{i}.

Getting there took \loopLemmas{} more lemmas in the prelude,
\loopLemmasAll{} proved: commutativity of addition, so that a checked
addition is recognised written either way round; \texttt{sub\_one\_lt},
which makes \texttt{xs[i - 1]} an index behind \texttt{i > 0}; and a
refuted comparison read as its converse.  \texttt{by\_bounds} learned to
carry each fact across a case split by \texttt{Eq.ind}, so that a
hypothesis \texttt{g or X} becomes \texttt{X} in the branch where
\texttt{g} is false, and to split on such a disjunction when a leaf is
stuck.
...
\begin{tabular}{lrrrr}
\toprule
port & functions & loops & obligations proved & corpus rows agreeing \\
\midrule
\input{gen/leanos_table.tex}
\end{tabular}
\end{center}

Each port gives its Python's answer on the Python model test's own
corpus, compiled by Crust and run.  They differ where the Python's integers
pass $2^{64}$: every \texttt{base + size} and \texttt{vaddr + memsz} in the
Python can wrap in a \texttt{u64}, and each is the checked form here --- a
region ending at the top of the address space followed by another is
\portWrapRefused{} rather than passed on a wrapped sum.  Three of the
theorems RosettaMath had proved only about hand-typed models are now
contracts proved about the lifted source: \texttt{reg\_class\_sized} and
\texttt{accept\_sized} on \texttt{elfcheck.rs}, \texttt{push\_keeps\_sp\_ok}
as the contract of \texttt{sp\_after\_push}, and \texttt{admit\_accepts} and
\texttt{admit\_disjoint} on \texttt{admit}.  The loader reads
\texttt{bases ++ vaddrs} in place rather than building it, since a
function returning a list is not in the fragment; the test checks it
against the Python's \texttt{regions\_disjoint} of the built list.  A
file names the files it calls into with a \texttt{// uses:} line; they are
appended after it, so its own line numbers, the ones every obligation
names, stay exact.  In \portSeconds{}\,s.

\section{The theorems LeanOS rests on}\label{sec:founding}

Everything above is a property of one function at a time.  LeanOS is
founded on two properties of the region list as a whole, and the LeanOS
paper proved them about a model of it typed out by hand~\cite{hartshorn2026leanos}:
\texttt{regions\_pairwise\_disjoint} --- if \texttt{regions\_disjoint}
says 1, then for every $j < k$ region $j$ ends at or before region $k$
begins --- and \texttt{region\_of\_unique} --- \texttt{region\_of} says 1
for at most one owner.  \texttt{memmap\_rs.py} proves both about
\texttt{memmap.rs} as lifted, \foundingProved{} of \foundingTheorems{}
theorems in \foundingSeconds{}\,s.

Most of the argument turned out not to need the model at all.  The hand
proof is about a \emph{specification}, \texttt{ordered\_prefix}, the
conjunction of the checker's guard at every position below a bound, and
the steps from it --- a guard at every position gives consecutive regions
in order, consecutive gives every pair, and two positions holding one
address are one position --- name no model.  They are run unchanged.  What
is proved afresh is where the Rust meets the specification.  The bridge,
\texttt{regions\_disjoint\_ordered}, goes through the lifted loop by
\texttt{by\_loop} with the invariant strengthened by
\texttt{ordered\_prefix} at the counter; \texttt{by\_bounds} opens the
specification one step at \texttt{i + 1} and rewrites it from the final
counter to the length by antisymmetry.  The port tests
\texttt{bases[i] - bases[i-1] < sizes[i-1]} behind
\texttt{bases[i-1] <= bases[i]} and the specification
\texttt{bases[i] < bases[i-1] + sizes[i-1]}; over the naturals they agree,
and \texttt{add\_le\_of\_le\_sub\_r} is the step.  The witness
\texttt{region\_index} returns is a Rust contract in the checked form,
proved by \texttt{rustprove} like any other, and read in the
specification's form by \texttt{lt\_add\_of\_sub\_lt}.
\texttt{region\_of\_unique} is then the hand model's composition proof, run
over the Rust's \texttt{region\_of}, \texttt{region\_index} and
\texttt{regions\_disjoint}, which have the Python's names and argument
order.

Three checks run with the proofs, so that the claim is not only in the
prose.  The shared theorems are stated here as in \texttt{memmap\_eq.py},
the same term: \foundingSame{}.  The \texttt{regions\_disjoint} under them
is the Rust's, its loop forming the checked subtraction and no sum:
\foundingIsRust{}.  And a Rust with the size check deleted --- a list with
overlapping regions accepted --- is \foundingMutant{} at the bridge, while
the same loop with \texttt{ordered\_prefix} left out of the invariant
\foundingMutantWhy{}: the proof fails exactly where the guard is missing.
Lean~\leanVersion{} accepted the exported file, \foundingLean{} theorems
depending on no axioms, in \foundingLeanSeconds{}\,s.
...
The loops found a second route to the same walk, inside the kernel's own
type checker.  To check \texttt{leb\_trans a b c} it normalised
\texttt{leb\_trans}'s type first, while \texttt{a}, \texttt{b} and
\texttt{c} were still variables --- and with a variable there is nothing to
compute, so \texttt{leb} opened into its recursor --- and substituted the
literal after, into a recursor with no \texttt{leb} left to fold.  The
type checker now normalises a function's type only if it is not already a
\texttt{Pi}, and a partial application's type only once its last argument
has arrived.  Argument types are compared by definitional equality as
before; only the order of substitution and normalisation moved.
...
\item \textbf{Not every theorem of the hand models, yet.}  The founding
  pair are proved about the Rust; two built on them are not yet:
  \texttt{sp\_no\_cross} in \texttt{threads\_eq.py} --- two threads whose
  stack pointers coincide, each in its own region, are one thread --- and
  \texttt{admit\_ordered} in \texttt{loader\_eq.py} --- an admitted
  image leaves the grown region list ordered.  The first is
  \texttt{region\_of\_unique} through \texttt{sp\_ok}; the second is the
  bridge again, over the concatenation \texttt{loader.rs} reads in place.
\item \textbf{Known gaps, asserted.}  A record contract through a field
  ...
  cannot happen quietly, and it is checked at runtime meanwhile.
\end{itemize}
